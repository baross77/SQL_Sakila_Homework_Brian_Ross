...
--3a. You want to keep a description of each actor. You dont think you will be performing queries 
	--on a description, so create a column in the table actor named description and use the data type BLOB 
	--(Make sure to research the type BLOB, as the difference between it and VARCHAR are significant).
alter table sakila.actor; 
add (Description blob default empty_blob()) varbinary(MAX);
select Description from sakila.actor;   
...
SELECT last_name,COUNT(*) AS cnt from sakila.actor GROUP by last_name HAVING cnt > 1;    
...
SELECT staff.last_name, staff.first_name, address.address FROM staff
INNER JOIN address ON
staff.address_id = address.address_id;

--6b. Use JOIN to display the total amount rung up by each staff member in August of 2005. Use tables staff and payment.
SELECT staff.last_name, staff.first_name, payment.amount, payment.payment_date, payment.staff_id FROM payment
JOIN staff USING (staff_id);
SELECT last_name, first_name, payment_date, amount, SUM(amount)
GROUP BY staff_id
HAVING payment_date between'$2005-08-01' AND '$2005-08-31';
...
SELECT title, COUNT(*) as num from film GROUP BY title;

--6d. How many copies of the film Hunchback Impossible exist in the inventory system?
SELECT 'Hunchback Impossilbe', (SELECT COUNT(*) FROM inventory 
...
WHERE language_id in SELECT language_idfilm = "English" AND title in film LIKE 'K%' or 'Q%';

SELECT title FROM film
WHERE film_id IN 
(SELECT film_id FROM film WHERE title LIKE 'K%' or 'Q%');
...
SELECT category.category_id, category.name , film_category.  FROM category JOIN film_category ON category.  =    
    
--8a. In your new role as an executive, you would like to have an easy way of viewing the Top five genres 
	--by gross revenue. Use the solution from the problem above to create a view. If you havent solved 7h, 
